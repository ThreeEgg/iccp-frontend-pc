  ...
  render() {
    ...
    return (
      <Sider
        ...
        className="case-info"
      >
        <div className="case-container flex flex-column">
          <div className="case-header">
            <Button className="case-back" type="primary" onClick={toggleCaseInfo}>
              返回消息列表
            </Button>
            {canSave && (
              <Button className="case-save" type="primary" onClick={this.saveCaseInfo}>
                {isEdit ? '保存' : '编辑'}
              </Button>
            )}
          </div>
          <div className="form-container flex-1">
            <Form name="caseForm" className="case-form" ref={this.formRef} layout="vertical">
              ...
              <Form.Item
                label={`债务人`}
                name="obligor"
                rules={[{ required: false, message: '请输入债务人!' }]}
              >
                <Input
                  maxLength="100"
                  disabled={!canSave || !isEdit}
              ...
              <Form.Item
                label={`国家/地区`}
                name="obligorCountry"
                rules={[{ required: false, message: '请输入国家/地区!' }]}
              >
                <Input
                  maxLength="100"
                  disabled={!canSave || !isEdit}
              ...
              <Form.Item
                label={`账龄`}
                name="ageOfAccount"
                rules={[{ required: false, message: '请输入账龄!' }]}
              >
                <Input
                  maxLength="100"
                  disabled={!canSave || !isEdit}
              ...
              <Form.Item
                label={`债务金额`}
                name="debtOfAmount"
                rules={[{ required: false, message: '请输入债务金额!' }]}
              >
                <Input
                  disabled={!canSave || !isEdit}
                  type="number"
              ...
              <Form.Item
                label={`货币类型`}
                name="currencyType"
                rules={[{ required: false, message: '请输入货币类型!' }]}
              >
                <Input
                  maxLength="100"
                  disabled={!canSave || !isEdit}
              ...
              <Form.Item
                ...
                rules={[{ required: false, message: '请输入案情简介!' }]}
              >
                <TextArea
                  maxLength="1000"
                  disabled={!canSave || !isEdit}
                    ...
                    file.isValid === 1 && (
                      <div className="file-item" key={index}>
                        <div className="file-box">
                          ...
                          <span className="file-name">{file.oldFileName}</span>
                        </div>
